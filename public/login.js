        ...
        .then(function(snapshot) {
            ...
            snapshot.forEach((item) => {
              if(item.val().userName == userone){
                document.getElementById("msg_alert").innerHTML = "<div class='alert alert-success'><strong>Welcome!</strong> User Recognized</div>"
            ...
            if(snapshot.numChildren() == 0 ){
                ...
                const ref = db.ref('users');
                ref.push(data).then((snap) => {
                  document.getElementById("msg_alert").innerHTML = "<div class='alert alert-success'><strong>Welcome!</strong> registered successfully</div>"
                  localStorage.setItem("id", snap.key);
                  localStorage.setItem("name", userone);
                  setTimeout(() => {
                    location.href = 'level.html';  
