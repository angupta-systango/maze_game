const characters ='ESNW';

function checkRightTrue(cell){

}

function checkTrueCount(cell){
  count = 0;
  active_dir = '';
  if(cell.e){
    active_dir = 'E';
...
function generateRandomDirection() {
    return characters.charAt(Math.floor(Math.random() * (+3 - +0)) + +0);
}
...
function displayLossMess() {
  document.getElementById("moves").innerHTML = "You Moved " + moves + " Steps.";
  toggleVisablity("Loss-Message-Container");  
} 
...
function Maze(Width, Height) {
  ...
  function defineStartEnd() {
    ...
    pacmanCoord = {
          x: Math.floor(Math.random() * (+8 - +1)) + +1,
          y: Math.floor(Math.random() * (+8 - +1)) + +1
        };
    switch (rand(4)) {
      case 0:
        startCoord = {
...
function Pacman(maze, c, _cellsize, onComplete, pacmanImg = null) {
  ...
  function changePacmanPos(pacmanRevisedCoords){
    if(pacmanRevisedCoords.x != maze.endCoord().x && pacmanRevisedCoords.y != maze.endCoord().y){
  ...
  setInterval(function(){   
    ...
    switch (pacmanLastStep){
      case "E":
            return_val = checkTrueCount(cell); 
            if(isNaN(return_val)){
              pacmanLastStep = return_val;
            }
            if (cell.e == true) {
              tempcoords = {
